...
resource "null_resource" "check_capacity_provider" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      if grep -q '"capacity_provider_name": {"value": "test1"' /var/lib/jenkins/workspace/test/terraform/modules/ecs_config/terraform.tfstate; then
...
resource "aws_ecs_cluster_capacity_providers" "example" {
  ...
  capacity_providers = [local.capacity_provider_exists ? "Capacity provider exists" : aws_ecs_capacity_provider.ecs_capacity_provider[0].name]

  default_capacity_provider_strategy {
    base              = 1
    weight            = 100
    capacity_provider = local.capacity_provider_exists ? "Capacity provider exists" : aws_ecs_capacity_provider.ecs_capacity_provider[0].name
  }
}
